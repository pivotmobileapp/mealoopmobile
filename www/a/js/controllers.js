...
// HOME - GET RECENT POSTS
	.controller('HomeCtrl', function($http, $scope, $rootScope, $state, $ionicLoading, PostService) {
	var datas = {};
	$scope.posts = [];
	$scope.arrr = [];
	$scope.brrr = [];
	...
	$scope.doRefresh = function() {
		...
		PostService.getRecentPosts(1)
			...
			console.log(data)
			datas.posts = data;
			datas.count_total = data.length;
			datas.count = data.length;
			$scope.totalPages = data.pages;
			$http.get('http://mealoop.com/main/wp-json/wp/v2/users')
				.success(function(user){
				angular.forEach(user, function (value, key) {
					$scope.arrr.push(value.name);
				})
				angular.forEach($scope.posts, function (valuee, key) {
					$http.get('http://mealoop.com/main/wp-json/wp/v2/comments?post='+valuee.id)
						.success(function(commentes){
								console.log(commentes.length)
						$scope.brrr.push(commentes.length);
								valuee.newname=$scope.arrr[key];
					valuee.comment_count=$scope.brrr[key];
					})
			...
			$scope.posts = PostService.shortenPosts(datas.posts);
    ...
    .then(function(data){
      ...
      $scope.totalPages = data.pages;
      var new_posts = PostService.shortenPosts(datas.posts);
...
// POST
	.controller('PostCtrl', function($http,$scope, post_data, $ionicLoading, PostService, AuthService, $ionicScrollDelegate) {
	var datas = {};
	var arr = [];
	$scope.users = [];

	datas.post = post_data;

	$scope.post = datas.post;

	$http.get('http://mealoop.com/main/wp-json/wp/v2/users/'+post_data.author)
		.success(function(user){

		angular.forEach(user, function (value, key) {

			$scope.post.newname  =user;

		})
	})


	$scope.comments = _.map(post_data.comments, function(comment){
		console.log(comment)
		if(comment.author_name){
			PostService.getUserGravatar(comment.id)
				.then(function(avatar){
				comment.user_gravatar = "http://www.gravatar.com/avatar/51845584f4c9270c65101307e203724b?s=150&#038;r=g&#038;d=wavatar";
			});
			return comment;
